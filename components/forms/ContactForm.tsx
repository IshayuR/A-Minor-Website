...
export function ContactForm() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 rounded-3xl border border-denim-100 bg-sky-50/80 p-6 shadow-soft backdrop-blur">
      ...
      <div>
        <label htmlFor="email" className="mb-1 block text-sm font-medium text-cocoa-700">Email</label>
        <input id="email" type="email" className="w-full rounded-xl border border-denim-200 bg-white/90 px-3 py-2 text-cocoa-700" aria-label="Enter your email" {...register("email")} />
        {errors.email && <p className="mt-1 text-xs text-red-700">{errors.email.message}</p>}
      </div>
      <div>
        <label htmlFor="eventType" className="mb-1 block text-sm font-medium text-cocoa-700">Event Type</label>
        <input id="eventType" type="text" className="w-full rounded-xl border border-denim-200 bg-white/90 px-3 py-2 text-cocoa-700" aria-label="Enter the event type" {...register("eventType")} />
        {errors.eventType && <p className="mt-1 text-xs text-red-700">{errors.eventType.message}</p>}
      </div>
      <div>
        <label htmlFor="message" className="mb-1 block text-sm font-medium text-cocoa-700">Event Details</label>
        <textarea id="message" rows={5} className="w-full rounded-xl border border-denim-200 bg-white/90 px-3 py-2 text-cocoa-700" aria-label="Enter event details" {...register("message")} />
